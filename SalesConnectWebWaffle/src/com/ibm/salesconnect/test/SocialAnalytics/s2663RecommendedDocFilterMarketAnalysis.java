...
/**
/**
 * Description   : [R1.3] Verify Recommended documents can filter for doc type 'IBM Market Analysis'.
 * @author KevinLau
 * @date 10/12/13
 */

public class s2663RecommendedDocFilterMarketAnalysis extends ProductBaseTest {
	
	Logger log = LoggerFactory.getLogger(s2663RecommendedDocFilterMarketAnalysis.class);
	...
	public void test_s2663RecommendedDocFilterMarketAnalysis() {
		
		log.info("Start of test method s2663RecommendedDocFilterMarketAnalysis");
		...
		try {			
			...
			log.info("Verify doc type filter 'IBM Market Analysis' exists");
			Assert.assertTrue(recdoc.verifyDocFilterExists(RecommendedDocsTab.docTypeMarketAnalysisLabel), 
					           "Did not find Doc type filter 'IBM Market Analysis'");
			
			log.info("Filter recommended docs with doc type filter 'IBM Market Analysis'");
			recdoc.selectDocTypeFilterOption(RecommendedDocsTab.docTypeMarketAnalysisLabel);
			
			log.info("Verify that filter returns only docs of type 'IBM Market Analysis'");
			Assert.assertTrue(recdoc.verifyDocTypeOfResults(RecommendedDocsTab.docTypeMarketAnalysisLabel),
								"Failed as doc types other than 'IBM Market Analysis' where found");
		...
		sugarAPI.deleteOpptySOAP(testConfig.getBrowserURL(), opptyID, user1.getEmail(), user1.getPassword());
		sugarAPI.deleteContactSOAP(testConfig.getBrowserURL(), contactID, user1.getEmail(), user1.getPassword());
		log.info("End of test method s2663RecommendedDocFilterMarketAnalysis");
	}
}